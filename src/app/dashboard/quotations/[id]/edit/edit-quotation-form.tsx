...
interface EditQuotationFormProps {
  ...
  existingItems: Tables<"quotation_items">[];
  parts: Tables<"parts">[];
  services: (Tables<"services"> & { category_name: string })[];
  customers: Tables<"customers">[];
}

export function EditQuotationForm({ quotation, existingItems, parts, services, customers }: EditQuotationFormProps) {
  const [isPending, startTransition] = useTransition();
  const [selectedItemIds, setSelectedItemIds] = useState<Record<number, string>>(() => {
    // Pre-populate selected item IDs from existing items
    const ids: Record<number, string> = {};
    existingItems.forEach((item, index) => {
      if (item.part_id) {
        ids[index] = item.part_id;
      } else {
  ...
  const {
    ...
    defaultValues: {
      ...
      vehicle_year: quotation.vehicle_year || undefined,
      vehicle_vin: quotation.vehicle_vin || "",
      vehicle_license_plate: quotation.vehicle_license_plate || "",
      vehicle_mileage: quotation.vehicle_mileage || undefined,
      driver_name: quotation.driver_name || "",
      tax_rate: quotation.tax_rate,
      discount_amount: quotation.discount_amount,
      notes: quotation.notes || "",
      items: existingItems.map((item) => {
        let mode: "inventory" | "manual" = "manual";
        if (item.type === "part") {
          mode = item.part_id ? "inventory" : "manual";
        } else {
          const serviceExists = services.some(s => s.name === item.description);
          mode = serviceExists ? "inventory" : "manual";
        }
        return {
          type: item.type,
          mode,
