...
export default async function NewQuotationPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-slate-900">New Quotation</h1>
        <p className="text-sm text-slate-500">Create a new quotation for a customer</p>
      </div>

      <Suspense fallback={<FormSkeleton fields={10} />}>
        <QuotationForm parts={parts} services={services} customers={customers} />
      </Suspense>
    </div>
  );
}
