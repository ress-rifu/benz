    ...
    const onSubmit = (data: CategoryFormData) => {
        startTransition(async () => {
            const result = isEditing
                ? await updateServiceCategory(category.id, data)
            ...
            if (result?.error) {
                ...
                toast({
                    ...
                    description: isEditing
                        ? t("forms.serviceCategoryUpdated")
                        : t("forms.serviceCategoryCreated"),
                });
                onOpenChange(false);
            }
        });
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>
                        {isEditing ? t("forms.editServiceCategory") : t("forms.addServiceCategory")}
                    </DialogTitle>
                    <DialogDescription>
                        {isEditing
                            ? t("forms.updateCategoryDetails")
                            : t("forms.addNewServiceCategory")}
                    </DialogDescription>
                </DialogHeader>
